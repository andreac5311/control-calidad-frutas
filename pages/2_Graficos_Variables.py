...
def cargar_datos():
    try:
        ...
        # Filtrar datos de variables continuas (con tilde)
        df = pd.read_sql("SELECT * FROM muestras WHERE tipo='Variable continua'", conn)
        conn.close()

        # Si no hay datos o no tiene las columnas esperadas, retornar DataFrame vacío
        if df.empty:
            return df

        # Verificar que existan las columnas de muestras
        ...
        # Filtrar solo variables continuas y seleccionar columnas relevantes
        df_filtrado = df[df['tipo'] == 'Variable continua'][columnas_requeridas].copy()
