...
st.set_page_config(page_title="Simulador de Datos", layout="wide")
...
PRODUCTOS_CONFIG = {
    "Mango": {
        "variables": {
            ...
            "Diametro (cm)": {"media": 8.5, "std": 0.4, "LIE": 7.5, "LSE": 9.5},
    ...
    "Aguacate": {
        "variables": {
            ...
            "Diametro (cm)": {"media": 7.0, "std": 0.5, "LIE": 6.0, "LSE": 8.5},
    ...
    "Melon": {
        "variables": {
            ...
            "Diametro (cm)": {"media": 15.0, "std": 1.0, "LIE": 12.0, "LSE": 18.0},
...
st.subheader("Configuracion de la simulacion")
...
with col2:
    nivel_variacion = st.select_slider(
        "Nivel de variacion del proceso",
...
st.markdown("---")
st.subheader("Parametros del proceso")
...
with col2:
    ...
    st.metric("Desviacion estandar", f"{std_sim:.3f}")
with col3:
...
if st.button("Generar y guardar datos simulados", use_container_width=True):
    ...
    col2.metric("Desviacion estandar", f"{np.std(X_bar, ddof=1):.3f}")
    col3.metric("Minimo", f"{np.min(datos_arr):.3f}")
    col4.metric("Maximo", f"{np.max(datos_arr):.3f}")

    st.info("Ve a Graficos de Variables para ver los graficos de control generados")

st.markdown("---")
st.subheader("Gestion de datos")
col1, col2 = st.columns(2)
with col1:
    if st.button("Borrar TODOS los datos", use_container_width=True, type="secondary"):
        conn = sqlite3.connect(DB_PATH)
